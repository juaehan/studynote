...
console.log("문자열의 길이 : " + len);  // 39

// 파라미터로 설정된 위치의 글자를 리턴한다.
// --> 위치는 0부터 카운트
const str2nd = msg.charAt(2);
console.log("두 번째 글자 : " + str2nd);    // f

// 모든 문자열은 그 자체로 배열처럼 취급될 수 있다.
console.log("두 번째 글자 : " + msg[2]);    // f

// 파라미터로 전달된 내용이 처음 나타나는 위치를 리턴한다.
const p1 = msg.indexOf("f");
console.log("'f'가 처음 나타나는 위치 : " + p1);     // 2

// 찾지 못할경우 -1을 반환함
console.log("'z'가 처음 나타나는 위치 : " + msg.indexOf("z"));      // -1
...
console.log("문자열 자르기 : " + substring1);       // Life is too short

// 두 번째 파라미터가 없을 경우 지정된 위치부터 끝까지 자른다.
const substring2 = msg.substring(19);
console.log("문자열 자르기 : " + substring2);       // You need Javascript.

// 모든 글자를 대문자로 변환한다.
const up = msg.toUpperCase();
console.log("모든 글자를 대문자 변환 : " + up);     // LIFE IS TOO SHORT, YOU NEED JAVASCRIPT.

// 모든 글자를 소문자로 변환한다.
const low = msg.toLowerCase();
console.log("모든 글자의 소문자 변환 : " + low);    // life is too short, you need javascript.

// 문자열의 앞/뒤 공백 지우기
const src1 = "   Hello World    ";
const src2 = src1.trim();
console.log(src1);      // ____Hello World____
console.log(src2);      // Hello World

// 문자열에 포함된 구분자를 기준으로 문자열을 잘라 배열로 반환하다.
