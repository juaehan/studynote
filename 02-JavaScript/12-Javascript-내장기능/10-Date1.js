...
const mm = date1.getMonth() + 1;    // 1+1 = 2
const dd = date1.getDate();         // 14

// 0=일요일, 6=토요일의 값이 리턴
const i = date1.getDay();           // 0~6
const day = days[i];                // 1 -> 월

const hh = date1.getHours();        // 17
const mi = date1.getMinutes();      // 23
const ss = date1.getSeconds();      // 1
...
/** 이미 생성된 날짜 객체의 성분 변경 */
date3.setYear(2010);
date3.setMonth(1);      // 0부터 시작하므로 2월을 위해서는 1로 설정
date3.setDate(14);
date3.setHours(12);
date3.setMinutes(16);
date3.setSeconds(30);
