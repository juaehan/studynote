...
// arr.includes(item, from)는 인덱스 from부터 시작해 item이 있는지 검색하는데, 해당하는 요소를 발견하면 true를 반환
...
// 요소의 위치를 정확히 알고 싶은게 아니고 요소가 배열 내 존재하는지 여부만 확인하고 싶다면 arr.inculdes를 사용하는게 좋다.
...
const results = arr4.filter(function(v, i, arr){
    ...
    if(v%2 == 0){
        // true가 리턴되는 경우 v는 results 배열의 원소로 저장
        // treu를 리턴하더라도 배열의 모든 원소를 탐색하기 전까지는 종료되지 않는다
        return true;
    } else{
        // false가 리턴되는 경우 v는 버려진다
        return false;
