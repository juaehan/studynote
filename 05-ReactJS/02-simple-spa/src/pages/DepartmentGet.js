 ...
 * (QueryString 방식으로 보안에 취약하기 때문에 Path 방식을 이용)
 */
...
const DepartmentGet = () => {
    ...
    console.debug(location);    // search: '?id=101&msg=hello' 가 필요함
    console.groupEnd();

    // 위의 QueryString을 객체 형태로 변환
    const {search} = location;
    const query = new URLSearchParams(search);
    console.group("QueryString 확인");
    console.debug(query);   // 객체들이 나옴
    console.groupEnd();

    // 추출된 변수값과 데이터 타입 확인
    console.debug('요청된 학과번호 값=%s (%s)', query.get('id'), typeof query.get('id'));   // 요청된 학과번호 값=101 (string)
    console.debug('요청된 메시지 내용=%s (%s)', query.get('msg'), typeof query.get('msg'));   // 요청된 메시지 내용=hello (string)
    console.groupEnd();
