...
class DBPool {
    ...
    static connectionInfo = {
        ...
        wqitForConnections: process.env.DATABASE_WAIT_FOR_CONNECTIONS,  // 커넥션 풀이 다 찬 경우 처리
        socketPath: process.env.DATABASE_SOCKET_PATH,
    };

    /** 싱클톤 객체를 생성하여 리턴하는 메서드 */
    static getInstance(){
    ...
    constructor(){
        ...
        this.pool.on('connection', (connection) => {
            ...
            connection.query = function (...args){
                ...
                // 3) 그 결과를 로그로 기록
                logger.debug(queryCmd.sql.trim().replace(/n/g," ").replace(/ +(?= )/g, " "));
                return queryCmd;
            }
        });
        ...
        this.pool.on('release', (connection) => {
            logger.info(` >> Connection 반남됨 [threaId=${connection.threadId}]`);
        });
    }
